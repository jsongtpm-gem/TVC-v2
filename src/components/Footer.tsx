...
const socials = [
  ...
  {
    label: 'Facebook',
    href: 'https://www.facebook.com/TorontoVintnersClub/',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
  ...
  {
    label: 'Instagram',
    href: 'https://www.instagram.com/torontovintners',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
  ...
  {
    label: 'LinkedIn',
    href: 'https://www.linkedin.com/company/toronto-vintners-club/',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
...
export default function Footer() {
  return (
    <footer className="bg-burgundy-900 text-ivory-200 mt-12 md:mt-20">
      <div className="max-w-6xl mx-auto px-6 py-10 md:py-12 flex flex-col md:flex-row gap-10 md:gap-16">
        <div className="flex-1">
          <p className="text-sm leading-relaxed text-ivory-200/80 mb-5">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            {socials.map(s => (
              <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
                aria-label={s.label}
                className="text-ivory-200/50 hover:text-gold-400 transition-colors">
                {s.icon}
              </a>
            ))}
            <a href="https://irongate.wine/" target="_blank" rel="noopener noreferrer"
              aria-label="Iron Gate Private Wine Management"
              className="transition-opacity group">
              <img src="https://irongate.wine/wp-content/uploads/2022/05/IronGate_HorizontalLogo_Colour.png"
                alt="Iron Gate Private Wine Management" className="h-5"
