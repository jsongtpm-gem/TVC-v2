...
export default function ContactEditor() {
  ...
  async function save() {
    ...
    await Promise.all(fields.map(({ key }) =>
      supabase.from('content').upsert({ page: 'contact', section: key, value: c[key] ?? '' }, { onConflict: 'page,section' })
    ))
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-10 space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-semibold text-burgundy-800" style={{ fontFamily: 'var(--font-playfair)' }}>Contact</h1>
      </div>
      {fields.map(({ key, label, multiline }) => (
        <div key={key}>
          <label className="block text-xs tracking-widest uppercase text-burgundy-600 mb-1.5">{label}</label>
          {multiline
            ? <textarea rows={3} value={c[key]??''} onChange={e=>setC({...c,[key]:e.target.value})} className="w-full border border-ivory-300 rounded-sm px-3 py-2.5 text-sm bg-white focus:outline-none focus:border-burgundy-700 transition-colors resize-none" />
